...
from multi_agent_implementation_demo.root_agent_implementation import run_root_agent
from shared.load_data import list_available_datasets

setup_paths()

# Try to import optional dependencies
try:
    from shared.load_data import download_and_save_dataset
except ImportError:
    def download_and_save_dataset(*args, **kwargs):
        return {"status": "error", "message": "Download function not available"}

try:
    from shared.benchmark_utils import ModelBenchmark
except ImportError:
...
def main():
    ...
    print("="*70)
    print("\nThis system uses a DYNAMIC agent structure:")
    print("  • Root Agent - Creates sub-agents dynamically based on task needs")
    print("  • Sub-agents - Created on-demand with ability to create their own tools")
    print("  • All agents can write and execute code dynamically")
    print("  • Minimal pre-written code - agents generate what they need")
    print("="*70)
    ...
    if available_datasets:
        ...
        print("⚠️  No datasets found locally.")
        print("   The agent can create tools to download datasets if needed.")
